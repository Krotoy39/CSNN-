...
class synapse:
    def __init__(self, kernel_size, kernel_num, train_or_test=0, G_name=r'', channels=3):
        # 下载突触器件的数据集
        self.LTP_s = np.array(pd.read_csv(r'folderpath\LTP.csv',
                                          header=None))         # 器件电导升高的过程
        self.LTD_s = np.array(pd.read_csv(r'folderpath\LTD.csv',
                                          header=None))         # 器件电导降低的过程
        ...
        # 输入和输出的维度决定了突触阵列的规模
        # 暂时先用一个卷积核
        self.kernel_size = kernel_size    # 卷积核的size是5
        ...
        self.G_unit = 1e-6                # 电导的单位是1e-4

        # 定义一个权重的改变量w,暂时为25*8,卷积核的size是5*5,卷积核的数目是8
        ...
        if train_or_test == 0:
            ...
            self.G = np.genfromtxt(G_name, delimiter=',')
        ...
        # 采样电阻设置为0.1S
        self.G_s = 0.1
        self.magnify = 2850     # 采样信号放大200倍,MNIST的(5,5)的卷积核采用450,cifar100采用28
        ...
        self.output_f_th = self.output_f_th_mean * np.ones(self.kernel_num)
    ...
    def cnn(self, V):
        ...
        # 将output做一个空array暂存
        image_size_1D, time = V.shape
        image_size = int(math.sqrt(image_size_1D))

        # 可以但是没必要而且会降低代码的复用性
        V = np.reshape(V, (image_size, image_size))
        ...
        return output, filed_time

    def cnn_CIFAR100(self, V):
    ...
    def dynamic_thresholds(self, filed):
    ...
    def SRDP_CIFAR100(self, filed, featuremap):
    ...
    def run_neurons(self, input_spikes):
        ...
        for i in range(time_step):
            ...
            if np.sum(self.firing) > 1:
                p = np.where(self.V_s == np.max(self.V_s))[0][0]
